...
public class Connection {
   ...
   */
  public func querySync(query:String) -> (result: [[NSDictionary]]?, error: [DBError]?) {
    var data = [[NSDictionary]]()
   ...
   */
  private func data_fetch(query: String, data: inout [[NSDictionary]]) -> (success: Bool, result: UnsafeMutablePointer<TABLE_RESULT>?) {
   ...
   */
  private func typecast (col: Int , value: String, result: inout UnsafeMutablePointer<TABLE_RESULT>?) -> NSDictionary {
    ...
    switch Int(col_data_type_fetch(Int32(col), result)) {
      default:
        #if os(Linux)
        let dict:NSDictionary = ["\(col_name)": String(value)].bridge()
        #else
        let dict:NSDictionary = [col_name as NSCopying: String(value) as AnyObject]
        #endif
